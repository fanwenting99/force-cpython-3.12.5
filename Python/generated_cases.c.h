...
// Do not edit!
        ...
        TARGET(POP_JUMP_IF_FALSE) {
            ...
            if (core_main_file_flag) {
                PyObject *cond = POP();
                int offset = (int)(next_instr - _PyCode_CODE(frame->f_code) - 1);
                int target_lineno = PyCode_Addr2Line(frame->f_code, oparg);
                int tmp_cond_choice;
                PyObject *forced_cond = NULL;
                if (target_lineno < current_lineno) {
                    ...
                    forced_cond = Py_False;
                    tmp_cond_choice = 0;
                }
                const char *filename_str = PyUnicode_AsUTF8(frame->f_code->co_filename);
                if (filename_str == NULL) { PyErr_Clear(); filename_str = "<?>"; }
                int record = check_or_write_record(filename_str, offset, opcode, oparg, tmp_cond_choice);
                if (record == -1) {
                    fork_and_exec_child(1, oparg);
                } else {
                    if (record == 1) forced_cond = Py_True;
                    else forced_cond = Py_False;
                }
                if (PyObject_IsTrue(forced_cond) == 0) {
                    JUMPBY(oparg);
                }
                Py_DECREF(cond);
                DISPATCH();
